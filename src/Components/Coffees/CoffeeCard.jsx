...
const CoffeeCard = ({ coffee, coffees, setCoffees }) => {
    // const [coffeees, setcoffees] = useState([])
    // const [cof, setCof] = useState(coffeees)
    const { _id } = coffee;
    const handleDelete = (_id)=>{
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://espresso-emporium-coffee-shop-server-crt5lkotn.vercel.app/coffees/${_id}`, {
                    method: 'DELETE'
    ...
    return (
        <div className="w-full">
            <div className="h-[19rem] w-full lg:pl-[1.81rem] rounded-xl bg-[#F5F4F1] flex justify-between items-center">
                <img className="pt-[1.94rem]" src={coffee.photo} alt="" />
                <div className="space-y-3 lg:mr-10">
                    <h1 className="railway lg:text-xl text-[#5C5B5B]"> <span className="text-[#1B1A1A] font-semibold">Name: </span> {coffee.name}</h1>
                    <h1 className="railway lg:text-xl text-[#5C5B5B]"> <span className="text-[#1B1A1A] font-semibold">Chef: </span>{coffee.chef}</h1>
                    <h1 className="railway lg:text-xl text-[#5C5B5B]"> <span className="text-[#1B1A1A] font-semibold">Price:</span> {coffee.price} TK</h1>

                </div>

                <div className=" pr-12">
                    <Link to={`/details/${coffee._id}`}>
                    <div className="w-[2.5rem] cursor-pointer h-[2.5rem] rounded-md bg-[#3C393B] flex justify-center items-center">
                    ...
                    <Link className="mt-3" to={`/update/${coffee._id}`}>
                    <div  className="w-[2.5rem] mt-3 cursor-pointer h-[2.5rem] rounded-md bg-[#EA4744] flex justify-center items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    ...
                    <div onClick={()=>handleDelete(coffee._id)} className="w-[2.5rem] mt-3 cursor-pointer h-[2.5rem] rounded-md bg-[#D2B48C] flex justify-center items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
